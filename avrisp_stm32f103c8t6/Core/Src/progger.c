...
void uart_sendChar(uint8_t tdat)
{
	...
	do result = CDC_Transmit_FS(tmp, 1);
...
void start_pmode() {
  ...
  HAL_Delay(20); // discharge PIN_SCK, value arbitrarily chosen
  reset_target(0);
  ...
  HAL_Delay(50); // datasheet: must be > 20 msec
...
void commit(unsigned int addr)
{
  ...
  if (PROG_FLICKER)
  {
    for(int i=0; i<PTIME; i++) HAL_Delay(1);
    prog_lamp(1);
  }
}
